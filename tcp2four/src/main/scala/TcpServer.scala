...
class TcpServerHandler(client: ActorRef, conn: Channel) extends Actor with ActorLogging {
  ...
  def receive = {
    ...
    // case TCPClose =>  println("TcpServerHandler TCPClose")
  }
}


class TcpServer(actorConsumer: ActorRef, listen: InetSocketAddress) extends Actor with ActorLogging {
  import Tcp._
    ...
    case c @ Connected(remoteAddresses, localAddresses) =>
      ...
      val handler =
        context.actorOf(Props(classOf[TcpServerHandler],
          actorConsumer, Channel(connId,conn)), "tcp_"+conn )
