...
class TcpClientHandler(f: ActorRef, remote: InetSocketAddress, conn:Channel, initial0: ByteString) extends Actor {
  ...
  // each channel handler will have own proxy
  //val bufferSize = 20
  //val proxyFlow = ProxyServer().buffer(bufferSize, OverflowStrategy.backpressure)
...
class TcpClient(actorConsumer: ActorRef, remote: InetSocketAddress) extends Actor with ActorLogging {
      ...
      pool.get(ch) match {
        ...
        case None => pool.put(ch,
          context.actorOf(Props(classOf[TcpClientHandler],
            actorConsumer, remote,ch,data), "tcp-client"+ch.connId))
      }
